...
var UserTableColumns = struct {
	...
	EmailVerificationToken: "users.email_verification_token",
	IsAdmin:                "users.is_admin",
	IsHidden:               "users.is_hidden",
...
var UserWhere = struct {
	...
	EmailVerificationToken whereHelpernull_String
	...
	EmailVerificationToken: whereHelpernull_String{field: "`users`.`email_verification_token`"},
	IsAdmin:                whereHelperbool{field: "`users`.`is_admin`"},
	IsHidden:               whereHelperbool{field: "`users`.`is_hidden`"},
...
type userR struct {
	UserSessions UserSessionSlice `boil:"UserSessions" json:"UserSessions" toml:"UserSessions" yaml:"UserSessions"`
}
...
func (r *userR) GetUserSessions() UserSessionSlice {
	if r == nil {
		return nil
	}
	return r.UserSessions
}
...
var (
	userAllColumns            = []string{"id", "username", "password_hash", "email", "email_verified", "email_verification_token", "is_admin", "is_hidden", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"username", "password_hash", "email", "email_verification_token", "deleted_at"}
	userColumnsWithDefault    = []string{"id", "email_verified", "is_admin", "is_hidden", "created_at", "updated_at"}
...
func (o *User) UserSessions(mods ...qm.QueryMod) userSessionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user_sessions`.`user_id`=?", o.ID),
	)

	return UserSessions(queryMods...)
}
...
func (userL) LoadUserSessions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_sessions`),
		qm.WhereIn(`user_sessions.user_id in ?`, argsSlice...),
		qmhelper.WhereIsNull(`user_sessions.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_sessions")
	}

	var resultSlice []*UserSession
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_sessions")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_sessions")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_sessions")
	}
...
// AddUserSessionsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserSessions.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddUserSessionsG(ctx context.Context, insert bool, related ...*UserSession) error {
	return o.AddUserSessions(ctx, boil.GetContextDB(), insert, related...)
}
